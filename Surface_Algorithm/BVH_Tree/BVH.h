...
#include "../AABB_Tree/TinyVector.h"
#include <queue>
#include <set>
...
private:
    ...
    Intersection getIntersection(const Ray& ray, BVH_Node* node, bool flag, int firstMeshFaceSize);
    Intersection getIntersection(const Ray& ray, BVH_Node* node, std::set<FH>& faceHandle);
...
public:
    ...
    // 重构，额外传入点的相邻面，检测面片相交时，无视这些面
    Intersection getIntersection(const Ray& ray, bool flag, int firstMeshFaceSize) {// 检查相交，并返回交点。交点index = -1 表示未相交s
        return getIntersection(ray, root, flag, firstMeshFaceSize);
    }
    // 重构，额外传入点的相邻面，检测面片相交时，无视这些面
    Intersection getIntersection(const Ray& ray, std::set<FH>& faceHandle) {// 检查相交，并返回交点。交点index = -1 表示未相交s
        return getIntersection(ray, root, faceHandle);
    }

    void buildBVH_Tree(MeshKernel::VolumeMesh&);
    void buildBVH_Tree(MeshKernel::SurfaceMesh&);
    //void buildBVH_Tree(MeshKernel::TetMesh&);
    //void buildBVH_Tree(SkeletalMesh&);
