...
//void BVH_Tree::buildBVH_Tree(SkeletalMesh& mesh) {
//
//    if (mesh.fsize() == 0) return;
//
//    std::vector<BVH_Node*> total_nodes;
...
Intersection BVH_Tree::getIntersection(const Ray& ray, const BVH_Face& face) {
    ...
    if (face.vertices.size() == 3) {
        ...
        if (!isec.happened) {
            std::vector<Vector3d> triangle1 = { face.vertices[0], face.vertices[2], face.vertices[3] };
...
Intersection BVH_Tree::getIntersection(const Ray& ray, BVH_Node* node) {
    ...
    isec.intersection_count = isec_left.intersection_count + isec_right.intersection_count;

    return isec;

}

Intersection BVH_Tree::getIntersection(const Ray& ray, BVH_Node* node, bool flag, int firstMeshFaceSize){
    // 检查射线是否与结点相交, 并返回交点s
    Intersection isec;
    ...
    if (node->left == nullptr && node->right == nullptr) {
        //无视点的相邻面
        if(flag){
            if(node->face.index < firstMeshFaceSize) return isec;
        }
        else{
            if(node->face.index >= firstMeshFaceSize) return isec;
        }
    ...
    Intersection isec_left, isec_right;
    if (node->left) isec_left = getIntersection(ray, node->left, flag, firstMeshFaceSize);
    if (node->right) isec_right = getIntersection(ray, node->right, flag, firstMeshFaceSize);
...
Intersection BVH_Tree::getIntersection(const Ray& ray, BVH_Node* node, std::set<FH>& faceHandle){
    // 检查射线是否与结点相交, 并返回交点s
    Intersection isec;
    ...
    if (node->left == nullptr && node->right == nullptr) {
        //无视点的相邻面
        if(faceHandle.count(FH{node->face.index})) return isec;
        isec = getIntersection(ray, node->face);
        return isec;
    }

    Intersection isec_left, isec_right;
    if (node->left) isec_left = getIntersection(ray, node->left, faceHandle);
    if (node->right) isec_right = getIntersection(ray, node->right, faceHandle);
