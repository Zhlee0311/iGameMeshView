...
void ViewOpenGL::initializeGL (void){
	...
	core->glClearColor (1.0, 1.0, 1.0, 1.0);
...
void ViewOpenGL::ViewTranslation(const QPoint &p){
    ...
    ViewTranslate(Vector3d(2.0 * dx / w * right / near_plane * z,
                           -2.0 * dy / h * top / near_plane * z,
                           0.0f));
}

void ViewOpenGL::ViewTranslate(const Vector3d &trans){
...
void ViewOpenGL::ViewAll (void){
	//makeCurrent();
    viewMatrix.setToIdentity();
    viewRotateMatrix.setToIdentity();
    tempViewRotateMatrix.setToIdentity();
...
void ViewOpenGL::ViewRotate(const QPoint& p) {
	...
	if (newPoint_hitSphere) {
		...
		QMatrix4x4 tempmatrix;
        QMatrix4x4 tempmatrix1;
		tempmatrix.translate(center[0], center[1], center[2]);	// 由于许多模型的初始center并不在坐标原点，需要把center先移到坐标原点再旋转
		tempmatrix.rotate(angle, axis[0], axis[1], axis[2]);
        tempmatrix1.rotate(angle, axis[0], axis[1], axis[2]);
		//viewRotateMatrix.rotate(angle, axis[0], axis[1], axis[2]);
		tempmatrix.translate(-center[0], -center[1], -center[2]);
		//viewMatrix = tempmatrix * viewMatrix;		// 左乘的方式，使动态欧拉角改成静态欧拉角，解决方向旋转的问题
		viewRotateMatrix = tempmatrix * viewRotateMatrix;
        tempViewRotateMatrix = tempmatrix1 * tempViewRotateMatrix;
	}
