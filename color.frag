...
// 颜色模式，1：光照方式（模型和光照位置分开，模型颜色会受旋转影响），2：根据法向方式（模型和光照位置绑定，模型颜色不受旋转影响），
// 3：自定义颜色（不受光照影响），4：使用传入VBO时使用的值作为颜色（不受光照影响）（默认传入的是顶点的向量）, 5：使用传入VBO时使用的值作为颜色（受光照影响）.
uniform int colormode = 1;
uniform vec4 color = vec4(1.0, 0, 0, 1.0);  // 自定颜色
void main() {
    vec3 n = normalize(cross(dFdx(pos),dFdy(pos))); // 默认离散
    ...
    if(colormode == 1){
        ...
        FragColor = vec4(inputColor * diff + ambient * 0.2, 1.0);
    }
}