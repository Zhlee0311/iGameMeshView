...
namespace MeshKernel {
    ...
    public:
        ...
        // 基类handle（iGameHandle）允许用int进行修改
        ...
        inline void idx(const int &_idx) { idx_ = _idx; }      //修改元素
    ...
    //继承自iGameHandle的其他类只能显式构造
    class iGameVertexHandle : public iGameHandle {
    public:
        explicit iGameVertexHandle(int _idx = -1) : iGameHandle(_idx) {}
    };

    class iGameEdgeHandle : public iGameHandle {
    public:
        explicit iGameEdgeHandle(int _idx = -1) : iGameHandle(_idx) {}
    };

    class iGameFaceHandle : public iGameHandle {
    public:
        explicit iGameFaceHandle(int _idx = -1) : iGameHandle(_idx) {}
    };

    class iGameCellHandle : public iGameHandle {
    public:
        explicit iGameCellHandle(int _idx = -1) : iGameHandle(_idx) {}
    };
}
