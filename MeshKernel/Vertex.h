...
namespace MeshKernel {
    ...
    public:
        ...
        iGameVertex(const iGameVertex &_v) : position(_v.position), normal(_v.normal) {}
        ...
        inline double dist(const iGameVertex &_rhs) const {                                   //计算两个向量的距离
        ...
        inline iGameVertex getNormal() {
            if (normal.size() == 3) return iGameVertex{normal[0], normal[1], normal[2]};
            return iGameVertex{0.f, 0.f, 0.f};
        }
...
namespace std {
    template<>
    struct hash<MeshKernel::iGameVertex> {
