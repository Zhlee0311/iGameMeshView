...
namespace MeshKernel {
	...
	public:
		/*=========================构造函数=============================*/
		iGameFace() :n_(0) {}//初始化：该面有0条边
		iGameFace(const std::vector<iGameVertexHandle>& _vertices, const std::vector<iGameEdgeHandle>& _edges) {
			assert(_edges.size() == _vertices.size());//断言顶点数要等于边数
			assert(_edges.size() >= 3);//断言边数要大于等于3
			n_ = _vertices.size();
		...
		const size_t size() const { return n_; }//返回面的边数
        ...
        MeshKernel::iGameVertex getNormal() {
            if (normal.empty()) {
                std::cout<<"error!: face normal is empty!"<<std::endl;
                return{0,0,0};
            }
...
{
	template<>
	struct hash <MeshKernel::iGameFace>
	{
